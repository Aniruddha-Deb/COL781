...
const glm::vec3 ORIGIN = glm::vec3(0.f, 0.f, -3.f);
...
class CornellBoxScene : public Scene
{
  ...
  public:
    CornellBoxScene(Camera& cam) : Scene(WIN_WIDTH, WIN_HEIGHT, cam, 1)
    {
        ...
        std::shared_ptr<Material> mirror_2 = std::make_shared<MetallicMaterial>(glm::vec3(0.2f, 0.3f, 0.2f));

        std::shared_ptr<Material> blue_wall_material = std::make_shared<DiffuseMaterial>(glm::vec3(.5f, .5f, .5f));
