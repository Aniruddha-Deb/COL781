...
namespace Software
{
    ...
    glm::vec3 phi_pc(glm::vec4 (&hom_tri)[3], glm::vec3 p, glm::vec2 pt)
    {
        ...
        return glm::vec3(p[0] / hom_tri[0].w, p[1] / hom_tri[1].w, p[2] / hom_tri[2].w);
    }
    ...
    void Rasterizer::drawObject(const Object &object)
    {
        ...
        auto render_triangle = [&](glm::ivec3 idxs) {
            ...
            for (auto [x, y, pt] : fragments)
            {
                ...
                float z = hom_tri[0].w * p_pc[0] / hom_tri[0].z + hom_tri[1].w * p_pc[1] / hom_tri[1].z +
                          hom_tri[2].w * p_pc[2] / hom_tri[2].z;
                // std::cout << z << std::endl;
