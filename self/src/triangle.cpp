...
int value_4xmsaa_square(glm::vec2 (&triangle)[3], int x, int y) {
    ...
    if (membership_check(triangle, pt1)) color += 50;
    if (membership_check(triangle, pt2)) color += 50;
    if (membership_check(triangle, pt3)) color += 50;
    if (membership_check(triangle, pt4)) color += 50;
...
int value_binary(glm::vec2 (&triangle)[3], int x, int y) {
    ...
    if (membership_check(triangle, pt)) return DARK_4;
    else return DARK_0;
}
...
void print(uint8_t (&display)[HEIGHT][WIDTH]) {
    ...
    for (auto r : display) {
        for (int i = 0; i<WIDTH; i++) {
            uint8_t c = r[i];
            if (c == DARK_4) cout << "█";
            else if (c == DARK_3) cout << "▓";
            else if (c == DARK_2) cout << "▒";
            else if (c == DARK_1) cout << "░";
            else cout << " ";
        }
        cout << '\n';
    }
...
int main(int argc, char** argv) {
    ...
    for (auto r : display) {
        for (int i=0; i<WIDTH; i++) r[i] = false;
    }
